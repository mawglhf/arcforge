...
def build_special_types_map(special_types_data: dict) -> dict:
        ...
        for level_key, items in levels.items():
            ...
            for item_entry in items:
                ...
                if item_name not in items_map:
                    items_map[item_name] = {"special_types": set(), "workshop_upgrades": [], "expedition_parts": [], "expedition_2_parts": [], "candlelight_parts": [], "quests": []}
    ...
    # Process expedition_2 (nested by part)
    expedition_2_data = special_types_data.get("expedition_2", {})
    for part_key, part_value in expedition_2_data.items():
        # Skip non-item parts
        if not isinstance(part_value, list):
            continue
        ...
        for item_entry in part_value:
            ...
            items_map[item_name]["special_types"].add("expedition_2")
            items_map[item_name]["expedition_2_parts"].append({
                "part": part_num,
                "quantity": quantity
            })
...
def adjust_item_data():
    ...
    for item in items_database:
        ...
        if item['name'] in special_types_map:
            ...
            if item_data['expedition_2_parts']:
                item['infobox']['expedition_2_parts'] = item_data['expedition_2_parts']
